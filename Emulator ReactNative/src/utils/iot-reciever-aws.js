...
class IotReciever extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.props.publishData) {
      if (
        prevProps.publishData.topic !== this.props.publishData.topic &&
        this.props.publishData.topic !== ''
      ) {
        this.mqttClient.publish(
          this.props.publishData.topic,
          this.props.publishData.message,
        );
  ...
  async componentDidMount() {
    ...
    this.mqttClient.mqttClientConnectHandler = () => {
      ...
      messageHistory = '';
      this.mount && this.props.callbackConnected(true);
      this.mount &&
        this.setState({
          connect: true,
        });
    ...
    this.mqttClient.mqttClientReconnectHandler = () => {
      ...
      this.mount && this.props.callbackConnected(false);
      this.mount &&
        this.setState({
          connect: false,
        });
    ...
    return (
      <View>
        <Icon
          name="dot-circle-o"
          size={24}
          color={this.state.connect ? this.props.headerColor : 'red'}
        />
      </View>
    );
  }
